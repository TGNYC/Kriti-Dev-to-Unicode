...
// input file
var fs = require('fs');
var textByLine = fs.readFileSync('input.txt').toString();

if (begin === 0) {
    legacy_text_var = textByLine;
} else {
    unicode_text_var = textByLine;
}
...
if (begin === 0) {
    var fs = require('fs');
    convert_to_unicode();
    fs.writeFileSync('output.txt', unicode_text_var)
} else {
    var fs = require('fs');
    Convert_to_Kritidev_010();
    fs.writeFileSync('output.txt', legacy_text_var)
}
